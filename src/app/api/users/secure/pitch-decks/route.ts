/**
 * API route for managing pitch decks in Client Tracker.
 * Handles GET, POST, PUT, DELETE requests for pitch deck CRUD operations.
 */
...
export async function GET(req: NextRequest) {
  try {
    ...
    const { data, error } = await supabase
      .from("pitch_decks")
      .select("id, encrypted_data, iv, salt, user_id")
      .eq("user_id", userId);

    if (error) {
      console.error("Supabase error:", error);
      return NextResponse.json(
        { error: "Failed to fetch pitch decks", details: error.message },
        { status: 500 }
      );
    }
    ...
    const validData = data.filter(
      (deck) =>
        deck.id && deck.user_id && deck.encrypted_data && deck.iv && deck.salt
    );
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { encryptedData: encrypted, iv, salt } = await encrypt(data, userKey);
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (error) {
      console.error("Supabase error:", error);
      return NextResponse.json(
        { error: "Failed to delete pitch deck", details: error.message },
        { status: 500 }
      );
    }
